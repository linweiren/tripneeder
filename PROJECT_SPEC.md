...
* 9B-1　Supabase migration 新增人設欄位：同行對象、預算感、體力、飲食禁忌（`profiles` 擴充或獨立 `user_persona` 表，啟動前需使用者確認）。
* 9B-2　選單新增「個性化設定」入口；新頁面可讀取 / 編輯 / 儲存人設。
* 9B-3　生成時：有人設 → 套用；無人設 → 用系統中性預設。
* 9B-4　系統中性預設：情侶 / 約會、一般預算、體力普通、無飲食禁忌。
...
依賴：9B 需先完成（否則沒人設只能用中性預設）。

* 9E-1　首頁預設「快速模式」：位置輸入 + 時長 chip。
* 9E-2　時長 chip：`1h` / `2h` / `3h` / `半天 (6h)` / `一天 (10h)` / `自訂`。
* 9E-3　時間起點 = 目前系統時間；結束 = 起點 + 選定時長。
* 9E-4　「進階設定」折疊區塊 / toggle：展開可填類型、預算、人數、tags、指定起訖時間。
* 9E-5　進階表單所有欄位皆選填，不阻擋送出（可以只填位置和時長直接送）。
* 9E-6　權重規則（**單欄覆寫**）：
  * 進階表單**有填**的欄位 → 這次使用進階值
  * 進階表單**沒填**的欄位 → 使用人設值
  * 人設也沒設 → 使用系統中性預設
* 9E-7　送出按鈕維持「出發！GO！（扣除 20 點數）」文案。

### 9F：局部編輯（節流 AI）

* 9F-1　詳情頁「刪掉這站」：純前端移除，重算時間軸與交通段，不叫 AI、不扣點。
* 9F-2　詳情頁「換掉這站」：新 API endpoint（`api/replace-stop.ts`）
  * 輸入：現有 plan + 要替換的 `stop.id` + 可選替換原因
  * 輸出：單一新 stop + 前後交通段更新
  * 扣 5 點
* 9F-3　換站前二次確認 + 扣點提醒。
* 9F-4　若替換結果涉及 Places 驗證，走 9D 驗證流程。
* 9F-5　換站前檢查點數餘額，不足阻擋。

### 9G：時間跨度過大 bug 修復

* 9G-1　重現「分析失敗但最近生成仍出現分析結果」的不一致來源（race condition / 錯誤處理順序）。
* 9G-2　修正 race condition 或錯誤處理順序。
* 9G-3　確保失敗不寫進最近生成、不扣點。
...
* 下一步：**Phase 9B 人設系統**（選單個性化設定，不強制）。啟動前需使用者確認 UI 細節與權重覆寫規則。
* 本地變更尚未推 production，可給組員測試的穩定版仍是 commit `bd11d8b`。9A-1/9A-2/9A-3 的所有新實作都在 worktree 分支（`claude/cranky-ride-9e2361`），尚未 merge 到 main、尚未發布。
