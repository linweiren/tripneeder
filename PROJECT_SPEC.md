...
* **9A**：AI 生成速度優化（prompt 瘦身 + streaming + 兩階段生成「骨架先出、細節後補」）。
* **9B**：人設系統（選單個性化設定，不強制）。
* **9C**：地理約束（第一站 2 km 內 + 交通時間軟規則 + 定位反查地名）。
* **9D**：Google Places 地點驗證（治幻覺，需寫 `docs/setup/google-places-setup.md` 教學；使用者完全不會用 API，要詳細說明申請與 `.env` 設定步驟）。
* **9E**：極簡輸入模式（首頁預設只問「在哪 / 多久」；進階表單折疊；單欄覆寫權重）。
* **9F**：局部編輯（詳情頁刪站 + 換站 API，換站扣 5 點）。
* **9G**：時間跨度過大的 bug 修復。
